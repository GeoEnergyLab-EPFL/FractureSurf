...
image_directory = 'D:\CT-Scan\CT-Scan Data\Core-CT-scan-M04\SlicesY-ID01-240125-M04';
...
for i = 2000:1:num_figures-80 % It has been eliminated with the CT-scan data
    i = 6400;
    ...
    if mod(i,10)==0
        ...
        current_frame.cdata=current_frame.cdata(1:400,1:1600,:);
    ...
    Roughness_Results = [Roughness_Results; [i*CT_reso, data_med, crest_value, data_mean, data_std, p_acf_lag(1), Pearson_Corr, ...
                                             H1, beta1, D1, real(gama1), H2, beta2, D2, real(gama2),...
...
writematrix([x_surface', ((y_surface-y_surface(end))*(-1))', z_surface1'],'M04_Fracture_Surfaces_Upper_real_position.csv');
writematrix([x_surface', ((y_surface-y_surface(end))*(-1))', z_surface2'],'M04_Fracture_Surfaces_Lower_real_position.csv');
writematrix(Roughness_Results,'M04_Roughness_Results.csv');

%% Load data from .csv
data_lower = csvread('M04_Fracture_Surfaces_Lower_real_position.csv');
data_upper = csvread('M04_Fracture_Surfaces_Upper_real_position.csv');
Roughness_Data = csvread('M04_Roughness_Results.csv');
...
zlabel('$$z\;(mm)$$','Interpreter','latex','FontSize',16);
ax = gca; 
ax.DataAspectRatio = [0.855 0.8 0.05]; 
view(2)
